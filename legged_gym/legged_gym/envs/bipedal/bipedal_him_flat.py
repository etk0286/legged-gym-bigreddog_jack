...
from .bipedal_him_config import BipedalHimFlatCfg

class BipedalHimFlat(LeggedRobot):
    cfg : BipedalHimFlatCfg
    ...
    def _compute_torques(self, actions):
        if self.cfg.control.use_actuator_network:
            ...
            if control_type == "MIXED_LIMBS_PT":
                ...
                tau_indices = self.cfg.control.torque_control_indices  # e.g., [2, 5]

                pos_action_scale = self.cfg.control.pos_action_scale     # e.g., 0.5
                torque_action_scale = self.cfg.control.torque_action_scale     # e.g., 1.0
                ...
                if len(tau_indices) > 0:
                    tau_idx = torch.tensor(tau_indices, device=actions.device)
                    tau_actions = actions[:, tau_idx] * torque_action_scale
                    torques[:, tau_idx] = tau_actions
